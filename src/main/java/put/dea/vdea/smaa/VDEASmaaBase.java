...
public abstract class VDEASmaaBase extends SmaaBase {
    ...
    public DataFrame<Double> calculateEfficiencyMatrix(ProblemData data, DataFrame<Double> inputValues, DataFrame<Double> outputValues) {
        var samples = generateWeightSamples(data);
        return calculateEfficiencyMatrixForSamples(inputValues, outputValues, samples);
    }

    @Override
    public double calculateEfficiency(DataFrame<Double> inputs, DataFrame<Double> outputs,
